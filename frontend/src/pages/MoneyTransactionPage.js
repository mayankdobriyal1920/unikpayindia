...
import {actionToGetMoneyTransactionData} from "../redux/CommonAction";
...
export default function MoneyTransactionPage() {
    ...
    const {loading,moneyTransactions} = useSelector((state) => state.userMoneyTransactions);
    ...
    const callFunctionToAddFilterAndGetData = (typeFilter, dateFilter) => {
        dispatch(actionToGetMoneyTransactionData(true, {type: typeFilter, created_at:dateFilter ? moment(dateFilter).format('YYYY-MM-DD') : null}))
    }

    useEffect(() => {
        setStatusTypeFilter('All');
        setDateTypeFilter(null);
        dispatch(actionToGetMoneyTransactionData())
    },[])
                    ...
                    <div className={"infiniteScroll"}>
                        <div className={"infiniteScroll__loading"}>
                            {loading ?
                                ...
                                : (moneyTransactions?.length) ?
                                    <div className={"sysMessage__container"}>
                                        <Virtuoso
                                            className={"virtual_item_listing"}
                                            totalCount={moneyTransactions?.length}
                                            itemContent={index => renderVirtualElement(moneyTransactions[index])}
                                        />
                                    </div>
                                    :
            ...
            <IonModal
                ...
                <IonContent className="ion-padding">
                    ...
                    <div className={"list_status_type"}>
                        ...
                        <div className={`list_status_type_item ${statusTypeFilter === 'game_play_deduct' ? 'active' : ''}`}
                             onClick={() => callFunctionToApplyTypeFilter('game_play_deduct', 'Game Play Deduct')}>Game Play Deduct
                        </div>
                        <div className={`list_status_type_item ${statusTypeFilter === 'game_percentage_deduct' ? 'active' : ''}`}
                             onClick={() => callFunctionToApplyTypeFilter('game_percentage_deduct', 'Game Percentage Deduct')}>Game Percentage Deduct
                        </div>
                        <div className={`list_status_type_item ${statusTypeFilter === 'wallet_to_game_wallet_transfer' ? 'active' : ''}`}
                             onClick={() => callFunctionToApplyTypeFilter('wallet_to_game_wallet_transfer', 'Wallet To Game Wallet Transfer')}>Wallet To Game Wallet Transfer
                        </div>
                    </div>
                </IonContent>
            </IonModal>
