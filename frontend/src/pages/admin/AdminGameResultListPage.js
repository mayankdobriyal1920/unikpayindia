...
export default function AdminGameResultListPage() {
    ...
    const callFunctionToAddFilterAndGetData = (typeFilter,dateFilter)=>{
        dispatch(actionToGetAdminGameResultListData(true,{status:typeFilter,created_at:dateFilter ? moment(dateFilter).format('YYYY-MM-DD') : null}))
    }
        ...
        setTimeout(()=>{
            ...
            if (shadowRoot) {
                ...
                buttons?.forEach((button) => {
                    button.style.fontSize = '1rem';  // Reduce font size
                ...
                icons?.forEach((icon) => {
                    icon.style.fontSize = '1rem';  // Reduce icon size
                });


                const weekDaysButtons = shadowRoot.querySelector('.calendar-days-of-week');
                if(weekDaysButtons)
                    weekDaysButtons.style.fontSize = '1rem';  // Reduce icon size
            }
        },100)
