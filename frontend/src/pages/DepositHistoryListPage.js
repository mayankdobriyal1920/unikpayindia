...
import {actionToGetDepositRequestHistoryData} from "../redux/CommonAction";
import LineLoaderComponent from "../components/LineLoaderComponent";
import { Virtuoso } from 'react-virtuoso'
export default function DepositHistoryListPage() {
    const history = useHistory();
    const [isTypeFilterOpen,setIsTypeFilterOpen] = useState(false);
    const [isDateFilterOpen,setIsDateFilterOpen] = useState(false);
    const [statusTypeFilter,setStatusTypeFilter] = useState('All');
    const {loading,depositHistory} = useSelector((state) => state.userDepositAmountHistory);
    ...
    useEffect(() => {
        setStatusTypeFilter('All');
        setDateTypeFilter(null);
        dispatch(actionToGetDepositRequestHistoryData())
    },[])
